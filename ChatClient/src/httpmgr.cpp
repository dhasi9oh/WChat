...
void HttpMgr::slotHttpFinish(ReqId id, QString res, ErrorCodes err)
{
    ...
    //json解析错误
    if(jsonDoc.isNull()){
        emit signalShowError(tr("json解析错误isNUll"));
        return;
    }
...
void HttpMgr::postHttpReq(QUrl url, QJsonObject json, ReqId req_id)
{
    ...
    //设置信号和槽等待发送完成
    QObject::connect(reply, &QNetworkReply::finished, [reply, this, req_id]() {
        //处理错误的情况
        if (reply->error() != QNetworkReply::NoError) {
            qDebug() << reply->errorString();
            //发送信号通知完成
            emit this->signalHttpFinish(req_id, "", ErrorCodes::ERR_NETWORK);
            reply->deleteLater();
            return;
        }
        ...
        //发送信号通知完成
        emit this->signalHttpFinish(req_id, res, ErrorCodes::SUCCESS);
        reply->deleteLater();
        });
}
